...
class ActionRejectSuggestion(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        response = requests.get(
            'http://fastapi-training1.herokuapp.com/pattern/get', params=params)
        ...
        for i in response:
            buttons.append(
                {"title": i['title'] + ": " + i['description'], "payload": "/pattern_id" + i['id']})
...
class ActionArchitectureSuggestion(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        # Ahora miro la opcion/intent que eligio el usuario. Si fue reject no seteo el slot
        if choosed_option == 'accept_suggestion':
